...
def get_employee_with_birthday_this_month():
    ...
    birthday_persons = frappe.db.sql("""select name, employee_name, date_of_birth
                                     ...
                                     order by date_of_birth ASC""" % conditions, as_dict=True)
    def get_day(data):
        print(data)
...
def get_presenty(department):
    members_present_toady = frappe.get_all('Attendance',filters={'attendance_date':nowdate(),'workflow_state':'Approved','status':['in',['Present','Work From Home','On Duty (OD)']],'department': department},fields=['employee_name'])
